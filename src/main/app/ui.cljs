...
(defkeyframes grow
  #_[:from {"stroke-dashoffset" 0
          :opacity 0}]
  [:to {"stroke-dashoffset" spiral-length}])
...
(defsc Wormhole [this
  ...
  {:query [:wormhole/status]
   :css [
         ...
         [:$spiral-wrapper [:$wormhole-grow-animation
                            ^:prefix {:animation "grow 5s linear"
                                      "stroke-dashoffset" 0
  ...
  (let [path-classes ["wormhole-grow-animation"]]
